...
int main() {
    ...
    while (cin >> secret) {
        ...
        while (n--) {
            cin >> guess;
            Counter secret_cnt_tmp = secret_cnt;
            ...
            for (size_t i = 0; i < kSize; ++i) {
                if (secret[i] == guess[i]) {
                    ++a;
                    secret_cnt_tmp[secret[i]]--;
                }
            }
            for (size_t i = 0; i < kSize; ++i) {
                if (secret[i] != guess[i] && secret_cnt_tmp.count(guess[i]) && secret_cnt_tmp[guess[i]] > 0) {
                    ++b;
                    secret_cnt_tmp[guess[i]]--;
                }
            }
            cout << a << 'A' << b << 'B' << '\n';
        }
